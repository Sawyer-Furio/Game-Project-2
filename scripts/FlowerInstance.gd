...
func update_visuals():
	...
	# 🌱 Set stem tile based on growth stage
	if growth_stage > 0 and growth_stage <= flower_data.stem_tile_ids.size():
		var stem_id = flower_data.stem_tile_ids[growth_stage - 1]
		stem_layer.set_cell(tile_pos, 0, stem_id)  # Use layer 0 for stem

	# 🌸 Set bloom tile if fully grown
	if growth_stage == 3 and flower_data.bloom_tile_id >= 0:
		bloom_layer.set_cell(tile_pos, 0, flower_data.bloom_tile_id)  # Use layer 0 for bloom
